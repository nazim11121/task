...
<div class="col-md-12">
  <h4 class="fw-bold py-3 mb-4">
    ...
    <a href="{{ route('slider.create') }}" class="btn btn-primary text-white float-right">Create</a>
  </h4>
</div>
<div class="card">
    ...
    <table id="example" class="table table-hover table-striped table-bordered">
      ...
      <tbody class="table-border-bottom-0">
        ...
        <?php foreach ($slider as $key => $value): ?>
          <tr>
        		<td>{{ ++$key }}</td>
        		<td><i class="fab fa-angular fa-lg text-danger"></i> <strong>{{ $value->slider_title }}</strong></td>
            <td><i class="fab fa-angular fa-lg text-danger me-1"></i> <strong><img id='img' src="{{ asset('storage/uploads/slider') }}/{{$value->image }}" style="width:230px; height:70px; margin-left:2px; margin-bottom: 10px"></strong></td>
            <td><i class="fab fa-angular fa-lg text-danger me-1"></i> <strong>{{ $value->description }}</strong></td>
            <td>
                <a class="btn btn-sm btn-primary" href="{{ route('slider.edit',$value->id) }}">Edit</a>
